...
import { LayoutDashboard, Gavel, BookOpen, Scale, Zap, Play, ChevronRight, Sparkles } from 'lucide-react';
import { Dashboard } from './components/Dashboard';
import { Courtroom } from './components/Courtroom';
import { Quiz } from './components/Quiz';
import { User } from './types';

const LOGO_URL = "https://img.icons8.com/color/512/justice.png"; // Premium Justice Icon
        ...
        <div className="w-full max-w-sm space-y-10 animate-in fade-in zoom-in duration-700">
          <div className="text-center logo-container">
            <div className="w-48 h-48 mx-auto mb-6 relative group">
              ...
              <div className="w-full h-full bg-[#1e293b] rounded-[2.5rem] flex items-center justify-center border-4 border-gold relative z-10 shadow-2xl overflow-hidden p-8">
            ...
            <p className="text-gold text-[10px] font-black uppercase tracking-[0.3em] opacity-80">Student Edition • Login</p>
          </div>
        ...
        <div className="w-full max-w-sm text-center space-y-12 animate-in fade-in slide-in-from-bottom-8 duration-700">
          ...
          <div className="glass p-8 rounded-[3rem] border border-white/10 relative overflow-hidden">
            <p className="text-slate-400 text-sm leading-relaxed mb-8 font-medium">
              Your neural link to the Tamil Nadu Legal Archive is active. Ready to proceed to the Arena?
            </p>
            <button
              onClick={() => setAuthState('app')}
              className="w-full py-6 bg-gold text-slate-900 font-black rounded-3xl shadow-3xl active:scale-95 transition-all transform uppercase tracking-[0.3em] text-sm flex items-center justify-center space-x-3 group hover:scale-105"
            >
      ...
      <div className="sticky top-0 z-50 glass-dark border-b border-white/5 px-6 py-4 flex justify-between items-center">
        <div className="flex items-center space-x-3">
          ...
          <h2 className="text-[10px] font-black text-white uppercase tracking-[0.2em]">
            {activeTab === 'dashboard' ? 'STATUS HUD' : activeTab === 'court' ? 'THE ARENA' : 'TRIALS'}
          </h2>
        </div>
        <div className="flex items-center space-x-3">
